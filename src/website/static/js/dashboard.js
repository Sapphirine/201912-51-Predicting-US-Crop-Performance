...
//  * Updated by nc2677 on 2019/11/05
//  */
...
function dashboard(id, fData) {
    ...
    fData.forEach(function (d) {
        // var debugText = d3.select(id)
        // .append("p").text(d3.sum(d3.values(d.count)));
        d.total = d3.sum(d3.values(d.count));                   /*TO FINISH*/
    });
    ...
    function histoGram(fD) {
        ...
        var numBars = fData.length;
        var barHorPad = 5;
        var barTextVertPad = 2;
        var barDist = (hGDim.w + hGDim.l + hGDim.r)/numBars; // bar distribution
        
        // debug output
        // var debugText = d3.select(id).data(fD)
        // .append("p").text(function (d) {return d;})
        // .append("p").text(fD[1][1]);
        // document.getElementById("debug").innerHTML = fData[0].count.ai;
        ...
        //////////////////////    DEBUG    /////////////////////////        
        // d3.select(id).data(fD).append("p").text(x.rangeBand());
        // document.getElementById("debug").innerHTML = x.range()[0];
        ////////////////////////////////////////////////////////////

        //create the rectangles.
        bars.append("rect")
            .attr("x", function(d,i){return x.range()[i];})      /*TO FINISH*/
            .attr("y", function(d,i){return  hGDim.h - d[1];})   /*TO FINISH*/
        ...
        bars.append("text").text(function (d) {
            ...
            .attr("x",function(d,i){                            /*TO FINISH*/
                return x.range()[i] + x.rangeBand() / 2;
            })          
            .attr("y", function(d,i){                           /*TO FINISH*/
                return  hGDim.h - (d[1] + barTextVertPad);
            })   
            .attr('text-anchor', 'middle');// anchor text to middle for transform
        ...
        hG.update = function (nD, color) {
            ...
            bars.select("rect").transition().duration(500)
                .attr("y",function(d){return  hGDim.h - d[1]})   /*TO FINISH*/
                .attr("height", function (d) {
                    // return hGDim.h - y(d[1]);
                    return d[1];
                })
